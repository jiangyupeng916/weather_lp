...
**核心目的：挂单提供流动性以获取平台返利奖励。** Polymarket 当前有挂单返利活动——只要在订单簿上挂限价单提供流动性，就能获得奖励。因此机器人只做纯 Maker 操作：挂限价买单提供流动性赚取奖励，成交后通过定时扫描持仓自动市价卖出平仓，始终保持资金在提供流动性的状态。
...
3. 对每个持仓：
   ...
   b. 跳过正在卖出中的持仓（并发保护）
   c. 查询链上余额确认持仓量
   d. 市价卖出（FOK）：SDK 自动算价，传 amount=shares，全成或全撤
```

**为什么用市价 FOK 而不是限价 GTC？**
- **快速平仓**：买入已成交的买单提供返利，持仓尽快了结
- **无部分成交残留**：FOK（Fill-or-Kill）要么全部成交要么全部撤销，不会留下零碎持仓需要下一轮再卖
- **无需关心卖出价**：市价由 SDK `calculate_market_price` 自动计算，按当前订单簿深度吃单
- **简单可靠**：不依赖 WS CONFIRMED → 立即卖出的复杂链路
- **自愈能力强**：重启、WS 断线重连、事件丢失等场景下，最多 120s 后自动重试

**市价卖出不再提供流动性返利**：买入侧挂买单赚取返利；卖出侧市价吃单不挂簿，无返利。这是策略上的取舍——用"无返利"换"快速平仓 + 无残留"。
...
6. **流程级防重复挂单（V7.5）**：不依赖事后发现，从流程上避免同一市场出现 2 个活跃订单
   - 下单异常时先查 open_orders 确认订单是否真的没挂上，再决定是否清幂等 token
   - 挂单前再次查 open_orders 确认无残留订单
   - 批量撤单失败也清空 active_id 进入冷却，让下轮 poll/audit 重查真实状态
